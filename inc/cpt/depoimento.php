...
function idc_depoimento_admin_column_content(string $column, int $post_id): void {
	if ($column === 'idc_dep_rating') {
		$r = (int) (function_exists('get_field') ? get_field('idc_dep_rating', $post_id) : 0);
		echo $r > 0 ? esc_html(str_repeat('★', $r)) : '—';
		return;
	}
	if ($column === 'idc_dep_role') {
		$role = function_exists('get_field') ? (string) get_field('idc_dep_role', $post_id) : '';
...
function idc_get_testimonials_from_cpt(): array {
	...
	foreach ($q->posts as $post) {
		...
		$quote = function_exists('get_field') ? (string) get_field('idc_dep_quote', $post->ID) : '';
		if ($quote === '' && $post->post_content !== '') {
			$quote = wp_strip_all_tags($post->post_content);
		}
		$rating = function_exists('get_field') ? (int) get_field('idc_dep_rating', $post->ID) : 5;
		$role   = function_exists('get_field') ? (string) get_field('idc_dep_role', $post->ID) : '';
...
function idc_upgrade_11213_testimonials_cpt(): void {
	...
	foreach ($items as $row) {
		...
		if (function_exists('update_field')) {
			update_field('idc_dep_rating', max(1, min(5, (int) ($row['rating'] ?? 5))), $post_id);
			update_field('idc_dep_quote', $quote, $post_id);
			update_field('idc_dep_role', (string) ($row['role'] ?? ''), $post_id);
		}
